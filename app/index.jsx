...
import { initializeDatabase } from "../utilities/db";
import {
  getAllActivities,
  addActivity,
  deleteActivity,
} from "../utilities/dboperation";
...
const index = () => {
  ...
  const handleAddCard = async (taskName, taskDescription, taskTime) => {
    try {
      await addActivity(taskName, taskDescription, parseInt(taskTime), 0); // Add to DB
      fetchCards(); // Refresh cards from DB
  ...
  const handleDelete = async (id) => {
    try {
      await deleteActivity(id); // Remove from DB
      fetchCards(); // Refresh cards from DB
    } catch (error) {
